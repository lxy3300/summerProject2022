...
if __name__ == "__main__":
    path_to_center = '../Halifax_data/Halifax_center/Halifax_center.shp'
    ...
    G_month = draw_graph(center, "../Halifax_image/graph_month.png", 'MRATE')
    draw_map(center, "../Halifax_image/map_month.png", 'MRATE')   # map corresponding to the graph
    Gm_json = json_graph.node_link_data(G_month)
    with open('../Halifax_data/graph_month.json', 'w') as f:
        json.dump(Gm_json, f)
    # draw the graph based on month rate
    G_day = draw_graph(center, "../Halifax_image/graph_day.png", 'DRATE')
    draw_map(center, "../Halifax_image/map_day.png", 'DRATE') # map corresponding to the graph
    Gd_json = json_graph.node_link_data(G_day)
    with open('../Halifax_data/graph_day.json', 'w') as f:
        json.dump(Gd_json, f)
    # draw the graph based on month rate
    G_hour = draw_graph(center, "../Halifax_image/graph_hour.png", 'HRATE')
    draw_map(center, "../Halifax_image/map_hour.png", 'HRATE')  # map corresponding to the graph
    Gh_json = json_graph.node_link_data(G_hour)
    with open('../Halifax_data/graph_hour.json', 'w') as f:
        json.dump(Gh_json, f)